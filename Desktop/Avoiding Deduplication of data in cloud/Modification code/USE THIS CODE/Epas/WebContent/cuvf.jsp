...
try
{
	...
	PreparedStatement ps=con.prepareStatement("select * from image where status='"+status+"'");
...
<body>
	...
	<div id="body">
		<div class="content">
			...
			<%=msg%>
			<%} %></center></p>
			<center><h3>View Images</h3></center>
			<h2>&nbsp;</h2>
			<center>
			<form >
			<table>
			...
			<%while(rs.next())
			{
				...
				<td><%=pp%></td>
				<td><%=t%></td>
				<td><a href="view1.jsp?a=<%=rs.getString("fname")%>&&b=<%=rs.getString("oname")%>&&c=<%=rs.getString("fid")%>">Send Request</a></td>
				</tr>
			<%} %>
